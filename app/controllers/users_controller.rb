class UsersController < ApplicationController 
    ...
    def destroy
        @user.destroy
        flash[:danger] = "Compte utilisateur et articles supprimés buddy"
    ...
    def require_same_user
        if current_user != @user and current_user.admin? == false
            flash[:danger] = "Vous n'êtes pas le bon utilisateur!"
    ...
    def require_admin
        if logged_in? and current_user.admin? == false
            flash[:danger] = "Action impossible si vous n'êtes pas admin"
            redirect_to users_path
        end
    end
end